...
eco.malecot2 <- function(eco, 
  ...
  if(method == "global") {  
    ...
    restricted <- (dist.sp >= dmin & dist.sp <= dmax) & (row(dist.sp)>col(dist.sp))
    ...
    if(testmantel.b) {
      ...
      obs <- cor(logdist.sp, Fij.sp)
      
      repsim <- numeric()
      N <- nrow(logdistmat)
      for(i in 1:nsim){
        samp <- sample(N)
        temp <- (kin[samp, samp])[restricted]
        repsim[i] <- cor(logdist.sp, temp)
      }
